...
x$.app = function( title, controller ) {
	...
	var chooseOption = function( controllerOption, defaultOption ) {
		return( controllerOption == undefined ? defaultOption : controllerOption );
	};
	
	// load defaults
	var defaults = {
		// application filters
		after: 	   chooseOption( controller.after, function(){console.log('before called')}),
		before:    chooseOption( controller.before, function(){console.log('after called')},
		// layouts and templating
		layout:    chooseOption( controller.layout, 'index.html'),  // default layout used
		container: chooseOption( controller.container, '#content'), // default element action results will be rendered in
		index: 	   chooseOption( controller.index, '_index.html'),  // default action to load
	};
	
	// apply action dispatch to all links
